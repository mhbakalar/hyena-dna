...
from src.dataloaders.datasets.mpra_dataset import MPRADataset
from src.dataloaders.datasets.mpra_dataset import MPRASeqDataset
from src.dataloaders.datasets.mpra_dataset import MPRASeqSSMDataset
...
class MPRA(HG38):
    ...
    def __init__(self, dataset_name, fasta_file, dest_path=None, tokenizer_name='char', d_output=None, rc_aug=False,
                max_length=1024, use_padding=True, max_length_val=None, max_length_test=None,
                padding_side='left', return_mask=False, val_ratio=0.0005, val_split_seed=2357, add_eos=False, 
                detokenize=False, val_only=False, batch_size=32, batch_size_eval=None, num_workers=1,
                shuffle=True, pin_memory=False, drop_last=False, fault_tolerant=False, ddp=False,
    ...
    def setup(self, stage=None):
        ...
        if self.tokenizer_name == 'char':
            ...
            self.tokenizer = CharacterTokenizer(
                characters=['A', 'C', 'G', 'T', 'N'],
                model_max_length=self.max_length + 2,  # add 2 since default adds bos/eos tokens, crop later
                add_special_tokens=False,
                padding_side=self.padding_side,
            )
...
class MPRASeq(HG38):
    _name_ = "mpra_agarwal_seq"
    l_output = 0  # need to set this for decoder to work correctly

    def __init__(self, dataset_name, kfold=None, dest_path=None, tokenizer_name='char', d_output=None, rc_aug=False,
                max_length=1024, use_padding=True, max_length_val=None, max_length_test=None,
                padding_side='left', return_mask=False, val_ratio=0.0005, val_split_seed=2357, add_eos=False, 
                detokenize=False, val_only=False, batch_size=32, batch_size_eval=None, num_workers=1,
                shuffle=True, pin_memory=False, drop_last=False, fault_tolerant=False, ddp=False,
                fast_forward_epochs=None, fast_forward_batches=None, return_names=False, *args, **kwargs):
    ...
    def setup(self, stage=None):
        ...
        # Create all splits: torch datasets (only train/test in this benchmark)
        dsets = ['val'] if self.val_only else ['train','val']
        datasets = [
            MPRASeqDataset(
                split=split,
                max_length=max_len,
            ...
            for split, max_len in zip(dsets, [self.max_length, self.max_length_val])
        ]
        if self.val_only:
            self.dataset_val = datasets[0]
        else:
            self.dataset_train, self.dataset_val = datasets

    def test_dataloader(self, *args: Any, **kwargs: Any) -> Union[DataLoader, List[DataLoader]]:
        """ The test dataloader, it's a dummy loader just to make the trainer happy, we don't use it."""
        return self._data_loader(self.dataset_val, batch_size=self.batch_size_eval)

class MPRASSMSeq(HG38):
    _name_ = "mpra_agarwal_seq"
    l_output = 0  # need to set this for decoder to work correctly
